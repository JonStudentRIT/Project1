...
// the two players
const player1 = { index: 1, active: true, selected: false };
const player2 = { index: 2, active: false, selected: false };
...
const searchWinCondition = (player) => {
  ...
  let win = false;
  // horizontal
  // linter wont accept the old format for this line for some reason but does accep this ¯\_(ツ)_/¯
  if (board[12] === player && board[13] === player
    && board[14] === player && board[15] === player) {
    win = true;
  }
...
const getWin = (request, response, params) => {
  ...
  if (params.player) {
    ...
    if (params.player === '1' || params.player === '2') {
      const didThisPLayerWin = searchWinCondition(params.player);
